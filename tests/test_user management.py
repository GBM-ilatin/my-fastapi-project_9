...
class TestUserManagement:
    ...
    def test_update_user_success(self, user_management, mock_user_repository, sample_user):
        ...
        # Act
        result = user_management.update_user(1, email="newemail@example.com", username="newusername")
    ...
    def test_update_user_ignore_user_id(self, user_management, mock_user_repository, sample_user):
        ...
        # Act
        result = user_management.update_user(1, user_id=999, email="newemail@example.com")

        # Assert
        assert result == sample_user
        assert sample_user.user_id == original_id  # Should not change
        assert sample_user.email == "newemail@example.com"

    def test_change_password_success(self, user_management, mock_user_repository, sample_user):
        ...
        with patch('bcrypt.checkpw', return_value=True) as mock_checkpw, \
             patch('bcrypt.hashpw', return_value=b'new_hashed_password') as mock_hashpw, \
             patch('bcrypt.gensalt', return_value=b'salt'):
            result = user_management.change_password(1, "oldpassword", "newpassword")

        # Assert
        assert result is True
        mock_user_repository.save.assert_called_once_with(sample_user)

    def test_change_password_user_not_found(self, user_management, mock_user_repository):
        """Test password change when user doesn't exist."""
        # Arrange
        mock_user_repository.find_by_id.return_value = None

        # Act
        result = user_management.change_password(999, "oldpassword", "newpassword")

        # Assert
        assert result is False
        mock_user_repository.save.assert_not_called()

    def test_change_password_wrong_old_password(self, user_management, mock_user_repository, sample_user):
        ...
        # Act
        with patch('bcrypt.checkpw', return_value=False):
            result = user_management.change_password(1, "wrongoldpassword", "newpassword")
